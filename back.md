...
Все эндпоинты сервиса данных доступны через gateway с общим префиксом `/data`.
...
CRUD-эндпоинты сервиса данных для заметок.

| Метод | Эндпоинт | Описание |
|-------|----------|----------|
| `POST` | `/data/note` | Создать новую заметку |
| `GET` | `/data/note/{id}` | Получить заметку по ID |
| `PUT` | `/data/note/{id}` | Обновить заметку |
| `DELETE` | `/data/note/{id}` | Удалить заметку |
| `GET` | `/data/notes` | Получить все заметки текущего пользователя |
...
| `GET` | `/data/notifications` | Список уведомлений |
